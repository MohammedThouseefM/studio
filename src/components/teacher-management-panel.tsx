            ...
            <TabsContent value="add-student">
              <Form {...studentForm}>
                <form onSubmit={studentForm.handleSubmit(onStudentSubmit)} className="space-y-6 pt-4">
                  ...
                  <FormField control={studentForm.control} name="photoUrl" render={({ field: { onChange, ...rest } }) => ( <FormItem> <FormLabel>Profile Photo</FormLabel> <div className="flex items-center gap-4"> <Avatar className="h-20 w-20 border"> <AvatarImage src={studentForm.watch('photoUrl')} alt="Student avatar" data-ai-hint="student portrait" className="object-cover"/> <AvatarFallback><User className="h-10 w-10" /></AvatarFallback> </Avatar> <div className="w-full"> <FormControl> <Input type="file" accept="image/*" onChange={(e) => { const file = e.target.files?.[0]; if (file) { const reader = new FileReader(); reader.onloadend = () => { onChange(reader.result as string); }; reader.readAsDataURL(file); } }} className="flex-1" {...rest} /> </FormControl> <p className="text-xs text-muted-foreground mt-2">Upload a picture for the student's profile.</p> </div> </div> <FormMessage /> </FormItem> )}/>
